...
#define eg_of(score)                                                           \
  static_cast<Score>(static_cast<int16_t>(((score) >> 16) & 0xFFFF))
...
constexpr PieceType piece_type(Piece pc) { return PieceType(int(pc) & 7); }

constexpr Piece mirror_piece(Piece pc) { return Piece(int(pc) ^ 6); }
